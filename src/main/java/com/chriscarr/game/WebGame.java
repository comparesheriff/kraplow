...
public class WebGame {
    ...
    public static int getCountPlayers(int gameId) {
        ...
        if (gamePreps.get(gameId) != null) {
            if (gamePreps.get(gameId).getLastUpdated() + (1000 * 60 * timeoutMintues)
    ...
    public static void start(int gameId, int aiSleepMs, Role pRole, Character pChar) {
        ...
        if (canStart(gameId)) {
            ...
            WebGameUserInterface x =
                new WebGameUserInterface(gamePreps.get(gameId).getJoinedPlayers(), aiSleepMs);
            webInit.setup(
                getCountPlayers(gameId), x, x, gameId, gamePreps.get(gameId).getSidestep(), pRole, pChar);
            gamePreps.remove(gameId);
        }
    }

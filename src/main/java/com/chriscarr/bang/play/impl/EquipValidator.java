...
public final class EquipValidator implements PlayValidator {
    @Override
    public ValidationResult validate(TurnContext ctx, PlayCommand cmd) {
        ...
        if (type != CardType.ITEM) {
            return ValidationResult.fail("card is not an item");
        }
        Player player = ctx.currentPlayer();
        if (!player.getHand().contains(card)) {
            return ValidationResult.fail("card not in hand");
        }
        if (player.isInPlay(card.getName())) {
            return ValidationResult.fail("card already in play");
        }
